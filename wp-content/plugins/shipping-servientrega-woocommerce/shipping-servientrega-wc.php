...
if(!defined('SHIPPING_SERVIENTREGA_WC_SS_VERSION')){
    define('SHIPPING_SERVIENTREGA_WC_SS_VERSION', '4.0.10');
}

if(!defined('SHIPPING_SERVIENTREGA_WC_SS_GUIDE_URL_API')){
    define('SHIPPING_SERVIENTREGA_WC_SS_GUIDE_URL_API', 'https://afternoon-castle-70263.herokuapp.com/api/guide');
}
...
function activate_shipping_servientrega_wc_ss() {
    shipping_servientrega_wc_ss()->DB_matriz();
}

function wp_upe_upgrade_completed( $upgrader_object, $options ) {
    $shipping_servientrega_plugin = plugin_basename( __FILE__ );
    if( $options['action'] == 'update' && $options['type'] == 'plugin' && isset( $options['plugins'] ) ) {
        foreach( $options['plugins'] as $plugin ) {
            if( $plugin == $shipping_servientrega_plugin ) {
                shipping_servientrega_wc_ss()->DB_matriz();
