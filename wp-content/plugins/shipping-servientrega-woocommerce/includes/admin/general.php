    ...
    try{
        ...
        if (!empty($license)){
            $params_guide = [
                'data' => $params,
                'auth' => [
                    'login' => $wc_main_settings['servientrega_user'],
                    'pwd' => $wc_main_settings['servientrega_password'],
                    'Id_CodFacturacion' => $wc_main_settings['servientrega_billing_code'],
            ...
            $response = wp_remote_retrieve_body($guide);
        ...
        if (strpos($exception->getMessage(), 'failed to load external entity') !== false)
            $message_error = '<small style="color:red">El proveedor de alojamiento esta bloqueando el puerto 8081, solicite que lo abran o bien resuelva actualizando a la </small> <a target="_blank" href="https://shop.saulmoralespa.com/producto/plugin-shipping-servientrega-woocommerce/">Versión completa</a>';
        update_option('servientrega_validation_error', $message_error);
        return false;
    }
